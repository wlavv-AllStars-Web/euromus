...
class SearchCore
{
    ...
    public static function getTags($db, $id_product, $id_lang)
    {
		...
		LEFT JOIN '._DB_PREFIX_.'tag t ON (pt.id_tag = t.id_tag AND pt.id_lang = '.(int)$id_lang.')
            ...
            $useCookie = true, Context $context = null)
    {
        ...
        if ($count) {
			...
			FROM
			`'._DB_PREFIX_.'tag` t
			STRAIGHT_JOIN `'._DB_PREFIX_.'product_tag` pt ON (pt.`id_tag` = t.`id_tag` AND pt.`id_lang` = '.(int)$id_lang.')
        ...
        $sql = 'SELECT DISTINCT p.*, product_shop.*, stock.out_of_stock, IFNULL(stock.quantity, 0) as quantity, pl.`description`, pl.`description_short`, pl.`link_rewrite`, pl.`name`, pl.`available_now`, pl.`available_later`,
