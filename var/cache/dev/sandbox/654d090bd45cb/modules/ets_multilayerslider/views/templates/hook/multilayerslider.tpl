...
<div class="ets_multilayerslider {if $mls_configs.ETS_MLS_SLIDER_TYPE=='full'}hidden{/if}">
    ...
    <div data-max-slide-time="{$mls_max_slide_time|intval}" class="mls_slider {if $mls_configs.ETS_MLS_ENABLE_LOADING_ICON}loading{/if} {if isset($mls_configs.ETS_MLS_SLIDER_TYPE)}mls_slider_type_{strtolower($mls_configs.ETS_MLS_SLIDER_TYPE)|escape:'html':'UTF-8'}{/if} mls_layout_{$mls_layout|escape:'html':'UTF-8'}" style=" {if isset($tc_config.YBC_TC_LAYOUT) && $tc_config.YBC_TC_LAYOUT == 'layouthome2'}width: 1170px; height: 460px; {else if isset($tc_config.YBC_TC_LAYOUT) && $tc_config.YBC_TC_LAYOUT == 'layouthome3'}width: 1937px; height: 522px; {else} width: 873px; height: 460px; {/if} {if $mls_configs.ETS_MLS_SLIDER_BACKGROUND}background-color: {$mls_configs.ETS_MLS_SLIDER_BACKGROUND|escape:'html':'UTF-8'};{/if}" {hook h='displayMLSConfigs'}>
        ...
        {if $mls_configs.ETS_MLS_ENABLE_LOADING_ICON}
            <div class="mls_loading_icon" style="{if $mls_configs.ETS_MLS_SLIDER_BACKGROUND}background-color: {$mls_configs.ETS_MLS_SLIDER_BACKGROUND|escape:'html':'UTF-8'};{else}#000000;{/if}"><img alt="" src="{$mls_img_base_dir|escape:'html':'UTF-8'}ajax-loader.gif"/></div>
        {/if}
        {if $mls_configs.ETS_MLS_ENABLE_NEXT_PREV}
            <div class="mls_nav">
                <div class="mls_prev" style="{if $mls_configs.ETS_MLS_SLIDER_BUTTON_COLOR}background-color: {$mls_configs.ETS_MLS_SLIDER_BUTTON_COLOR|escape:'html':'UTF-8'};{/if}">{l s='Prev' mod='ets_multilayerslider'}</div>
                <div class="mls_next" style="{if $mls_configs.ETS_MLS_SLIDER_BUTTON_COLOR}background-color: {$mls_configs.ETS_MLS_SLIDER_BUTTON_COLOR|escape:'html':'UTF-8'};{/if}">{l s='Next' mod='ets_multilayerslider'}</div>
            </div>
        {/if}
        {if $mls_configs.ETS_MLS_ENABLE_PAGINATION}
            <div class="mls_pagination">
                ...
                {foreach from=$mls_slides item='slide'}
                    {assign var='ik' value=$ik+1}
                    <div class="mls_pag_button mls_pag_{$ik|intval}" style="background-color: {if isset($mls_configs.ETS_MLS_SLIDER_BUTTON_COLOR) && $mls_configs.ETS_MLS_SLIDER_BUTTON_COLOR}{$mls_configs.ETS_MLS_SLIDER_BUTTON_COLOR|escape:'html':'UTF-8'}{/if}" data-slide-order="{$ik|intval}" data-id-slide="{$slide.id_slide|intval}">{$ik|intval}</div>
                {/foreach}
            </div>
        {/if}
    </div>
