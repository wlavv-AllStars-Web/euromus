...
class CatalogControllerCore extends FrontController {
    ...
    public function initContent() {
        ...
        $manufacturers = $this->setUpdated($temp_manufacturers);

        // Assign manufacturers to Smarty
    ...
    public function setUpdated($manufacturers) {
        ...
        foreach ($manufacturers as $manufacturer) {
            $dates = [
                $manufacturer['xlsx_update'],
                $manufacturer['pictures_update'],
                $manufacturer['logos_update'],
                $manufacturer['facebook_update'],
                $manufacturer['site_update']
            ];
            ...
            $sql = 'UPDATE eu_manufacturer SET 
                csv=' . $csv_status . ', 
                xlsx=' . $xlsx_status . ', 
                pictures=' . $pictures_status . ', 
                logos=' . $logos_status . ', 
                info=' . $info_status . ', 
                info_updated="' . $mostRecent . '", 
                facebook=' . $facebook_status . ', 
                site=' . $site_status . ' 
                WHERE id_manufacturer=' . $manufacturer['id_manufacturer'];
