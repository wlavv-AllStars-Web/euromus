...
class Ybc_widget extends Module
{
	...
	{	   
        ...
        $this->_hooks = array(
            ...
            array(
                'hook' => 'ybcCustom1',
                'name' => $this->l('Banner under "popular product"')
            ),
            array(
                'hook' => 'ybcCustom2',
                'name' => $this->l('Banner under "Best Selling product"')
            ), 
            array(
                'hook' => 'ybcCustom3',
                'name' => $this->l('Banner under "Special product"')
            ),
            array(
                'hook' => 'ybcCustom4',
    ...
    public function getFieldsValues($formFields, $primaryKey, $objClass, $saveBtnName)
	{
        ...
        foreach($formFields as $field)
        {
            if(!isset($field['primary_key']) && !isset($field['multi_lang']) && !isset($field['connection']))
            {
                ...
                $fields[$field['name']] = Tools::getValue($fieldName, $obj->$fieldName) ? trim(Tools::getValue($fieldName, $obj->$fieldName)) : Tools::getValue($fieldName, $obj->$fieldName);
            }
