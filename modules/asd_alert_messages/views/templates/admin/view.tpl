    ...
    <div class="col-lg-3">
        <div class="panel">
    		...
    		<div class="panel-body" style="padding: 0;">
		        ...
		        {if count($messages) > 0}
        		    {foreach $messages AS $message}
        		        <div style="border-bottom: solid 1px #eee;display: inline-block;width: 100%;">
        		            <div style="width: 30px;float: left;" class="btn btn-danger" onclick="deleteAlert({$message['id']})"><i class="icon-trash"></i></div>
        		            <div style="width: calc( 100% - 30px );float: left; font-size: 16px; padding-left: 5px; padding-top: 5px;cursor: pointer;" onclick="openAlert({$message['id']})">{$message['title']}</div>
        		            <div style="display: none;">
        		                ...
        		                <input type="hidden" id="message_type_{$message['id']}"   value="{$message['message_type']}"></span>
        		                <input type="hidden" id="message_status_{$message['id']}" value="{$message['message_status']}"></span>
        		            </div>
        		        </div>
    		        {/foreach}
    		    {else}
    		    ...
    		    <div>
    		        <button class="btn btn-success" type="button" onclick="newAlert()" style="text-transform: uppercase; font-size: 16px; width: 100%;margin-top: 20px;">
    		            Create new message
    		        </button>
    		    </div>
        ...
        <div class="panel">
    		<div class="panel-heading text-center" style="border: 0px solid black;">
    			<div style="margin: 10px 25px;">
    			    <span style="float: right;"><button type="button" class="btn btn-info" onclick="saveFormData()" id="save_button">Save</button></span>
    			    <span style="float: left;margin: 5px; font-size: 20px;" id="title_panel"></span>
        		</div>
    		</div>
    		<div class="panel-body" style="padding: 0 15px;">
    		    <form action="/admin77500/index.php?controller=AdminDashboard&token={$token}&save_dashboard_message=1" method="post">
    		        <input type="hidden" value="0" id="dashboard_message_id" name="message_id">
            		<div class="row">
            		    
            		    <div class="col-lg-6">
                		    <input placeholder="Message title" type="text" id="dashboard_message_title" name="dashboard_message_title" value="" onchange="$('#title_panel').text($(this).val())">
            		    </div>
            		    
            		    <div class="col-lg-3" style="display: none;">
                		    <select name="dashboard_message_type" id="dashboard_message_type" onchange="set_message_type($(this).val())">
                		        <option {if Configuration::get('dashboard_message_type_1') == 1} selected="selected" {/if} value="1">Danger</option>
                		        <option {if Configuration::get('dashboard_message_type_1') == 2} selected="selected" {/if} value="2">Warning</option>
                		        <option {if Configuration::get('dashboard_message_type_1') == 3} selected="selected" {/if} value="3">Success</option>
                		        <option {if Configuration::get('dashboard_message_type_1') == 4} selected="selected" {/if} value="4">Info</option>
            		        </select>
            		    </div>
            		    
            		    <div class="col-lg-3">
            		        <input style="margin-top: 8px;" type="checkbox" id="enable_dashboard_message_type" name="enable_dashboard_message_type" {if Configuration::get('enable_dashboard_message_type') == 1} checked="checked" {/if} value="1"><span style="margin-left: 5px;">Enable message</span>
            		    </div>
            		    ...
            		    <div class="col-lg-6">
                            ...
                            <textarea class="textarea_input" name="dashboard_message_en" id="dashboard_message_en" oninput="set_message($(this).val())"></textarea>
            		    </div>
            		    <div class="col-lg-6">
                            <div class="input-group-prepend">ES</div>
                            <textarea class="textarea_input" name="dashboard_message_es" id="dashboard_message_es" oninput="set_message($(this).val())"></textarea>
            		    </div>
            		    ...
            		    <div class="col-lg-6">
                            ...
                            <textarea class="textarea_input" name="dashboard_message_fr" id="dashboard_message_fr" oninput="set_message($(this).val())"></textarea>
            		    </div>
            		    <div class="col-lg-6">
                            <div class="input-group-prepend">RO</div>
                            <textarea class="textarea_input" name="dashboard_message_ro" id="dashboard_message_ro" oninput="set_message($(this).val())"></textarea>
            		    </div>
            		    ...
            		    <div class="col-lg-6">
                            ...
                            <textarea class="textarea_input" name="dashboard_message_pt" id="dashboard_message_pt" oninput="set_message($(this).val())"></textarea>
            		    </div>
            		    
            		    
            		    <div class="col-lg-6">
                            <div class="input-group-prepend">IT</div>
                            <textarea class="textarea_input" name="dashboard_message_it" id="dashboard_message_it" oninput="set_message($(this).val())"></textarea>
            		    </div>
            		    
            		    <div class="col-lg-12"> </div>

            		    <div class="col-lg-12" style="display: none;"> <h1 style="border: 0px solid black;">Preview</h1>
                		    <div style="min-height: 50px;" id="dashboard_preview_message" class="alert alert-info"</div>
            		    </div>
...
<style>
    .spacer-1{ width: 100%; height: 1px; display: inline-block; }
    .spacer-5{ width: 100%; height: 5px; display: inline-block; }
    .spacer-10{ width: 100%; height: 10px; display: inline-block; }
    .spacer-20{ width: 100%; height: 20px; display: inline-block; }
    .textarea_input{ height: 100px !important; width: calc( 100% - 30px ) !important; float: left; }
    .input-group-prepend{ height: 100px; line-height: 6;width: 30px; float: left; padding: 3px 5px; font-size: 16px; background-color: #dfe7ea; border: 1px solid #C7D6DB; }
    #save_button{ width: 50px; font-size: 16px; text-transform: uppercase;margin-top: 5px; }
