    ...
    public function getRates($destCountry, $destRegion, $city, $destPostCode, $baseSubtotalInclTax, $weight, $boxes, $insuranceValue, $oldResults = array(), $iteration = 1)
    {
        ...
        if ($helper->isEnabledCustomMapping()) {
            ...
            if ($this->useCustomRates($destCountry)) {
                ...
                $values = json_encode(array($destCountry, $destRegion, $destPostCode, $weightString, $boxes, $insuranceValue));
                $cache_id = 'Carrier::getCarriers_Shipments' . md5($values);
                $cookie_key = 'Carrier::getCarriers_Shipments_MBE';

                if (\Context::getContext()->cookie->__isset($cookie_key)) {
                    $cookie_value = \Context::getContext()->cookie->__get($cookie_key);
                    if (isset($cookie_value)) {
                        $cookie_value = json_decode($cookie_value);
                        if ($cookie_value->cache_id == $cache_id) {
                            $shipments = $cookie_value->shipments;
                        } else {
                            \Context::getContext()->cookie->__unset($cookie_key);
                        }
                    } else
                        \Context::getContext()->cookie->__unset($cookie_key);
                }
                ...
                if (!isset($shipments) || $shipments === false) {
                    $shipments = $ws->estimateShipping($destCountry, $destRegion, $destPostCode, $weight, $boxes, $insuranceValue, $allowedShipmentServicesArray);
                    ...
                    \Context::getContext()->cookie->__set($cookie_key, json_encode($cookie_value));
